...
/*----- liệt kê danh sách product cat con dựa vào Product Cat Id cha gần nhất -------*/
public function getProductCat_byProductCatIdParent_home($idProductCatParent,$order_productCatId){
...
public function getProductFavoriteList_limit ($limit) {
    global $conn_vn;
    $sql = "SELECT * FROM product WHERE product_favorite = 1 LIMIT $limit";
    $result = mysqli_query($conn_vn, $sql);
    $rows = array();
